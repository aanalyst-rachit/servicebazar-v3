...
export const searchAddressOnMap = async (
  ...
  if (!address.trim()) {
    Alert.alert('Alert ⚠️', 'Address input me type karein!');
    return;
  }
  setLoadingMap(true);
  try {
    ...
    if (data && data.length > 0) {
      ...
      Alert.alert('Success 📍', 'Map location update ho gayi!');
    } else {
      Alert.alert('Location Not Found ❌', 'Sahi location name enter karein.');
    }
  } catch {
